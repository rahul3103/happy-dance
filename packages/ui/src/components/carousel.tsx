...
function Carousel({
  ...
  const onSelectDotButton = React.useCallback((api: CarouselApi) => {
    if (!api) return;
  ...
  React.useEffect(() => {
    ...
    onInit(api);
    onSelectDotButton(api);
    api
      .on("reInit", onInit)
      .on("reInit", onSelectDotButton)
      .on("select", onSelectDotButton);
  }, [api, onInit, onSelectDotButton]);
...
function CarouselContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div ref={carouselRef} data-slot="carousel-content">
      <div
        className={cn("flex touch-pan-y touch-pinch-zoom -ml-4", className)}
        {...props}
      />
    </div>
  );
...
function CarouselItem({
  ...
  return (
    <div
      ...
      className={cn(
        "shrink-0 grow-0 transform-gpu flex-none basis-full min-w-0 pl-4  transition-opacity",
    ...
    <div {...props} className="flex justify-center mt-6">
      {scrollSnaps.map((_, index) => (
        <DotButton
          ...
          className={cn(
            index === selectedIndex ? "bg-brand-orange" : "bg-muted"
          )}
        />
      ))}
    </div>
